...
Wichitakey <- df41[survey_date=="2018",.(state_code, `6_digit_id`, county_code, place_name, cbsa_code)][cbsa_code=="48620"]


Wichita_summary <- df41 %>%
  filter(cbsa_code == "48620") %>%
...
View(Wichita_summary)
temp <- Wichita_summary[-1] %>%  as.matrix() %>% t() 
colnames(temp) <- Wichita_summary$survey_date
temp <- temp  %>% as_tibble() %>% mutate(var=names(Wichita_summary)[-1] ) %>% as.data.table()
temp41 <- temp[,c("var",Wichita_summary$survey_date), with=FALSE]
View(temp41)

df38 <- rbindlist(test38)

Wichita_summary <- df38 %>%
  filter(state_code %in% unique(Wichitakey$state_code) &
         county_code %in%unique(Wichitakey$county_code) &
         `6_digit_id` %in% unique(Wichitakey$`6_digit_id`)) %>% 
  group_by(survey_date) %>%
  summarise(bldg1 = sum(`1_unit_bldgs`), unit1 = sum(`1_unit_units`), value1 = sum(`1_unit_value`), 
            bldg2 = sum(`2_unit_bldgs`), unit2 = sum(`2_units_units`), value2 = sum(`2_units_value`),
            bldg3_4 = sum(`3_4_units_bldgs`), unit3_4 = sum(`3_4_units_units`), value3_4 = sum(`3_4_units_value`),
...
colnames(temp) <- Wichita_summary$survey_date
temp <- temp  %>% as_tibble() %>% mutate(var=names(Wichita_summary)[-1] ) %>% as.data.table()
temp38 <- temp[,c("var",Wichita_summary$survey_date), with=FALSE]
View(temp38)

df35 <- rbindlist(test35)
Wichita_summary <- df35 %>%
  filter(state_code %in% unique(Wichitakey$state_code) &
           county_code %in%unique(Wichitakey$county_code)) %>%  # `6_digit_id` %in% unique(Wichitakey$`6_digit_id`)
  group_by(survey_date) %>%
  summarise(bldg1 = sum(`1_unit_bldgs`), unit1 = sum(`1_unit_units`), value1 = sum(`1_unit_value`), 
            bldg2 = sum(`2_unit_bldgs`), unit2 = sum(`2_units_units`), value2 = sum(`2_units_value`),
            bldg3_4 = sum(`3_4_units_bldgs`), unit3_4 = sum(`3_4_units_units`), value3_4 = sum(`3_4_units_value`),
...
colnames(temp) <- Wichita_summary$survey_date
temp <- temp  %>% as_tibble() %>% mutate(var=names(Wichita_summary)[-1] ) %>% as.data.table()
temp35 <- temp[,c("var",Wichita_summary$survey_date), with=FALSE]
View(temp35)
